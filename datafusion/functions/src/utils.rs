        ...
        ($FUNC:expr, $ARGS:expr, $EXPECTED:expr, $EXPECTED_TYPE:ty, $EXPECTED_DATA_TYPE:expr, $ARRAY_TYPE:ident) => {
            ...
            match expected {
                Ok(expected) => {
                    ...
                    let result = func.invoke_with_args(datafusion_expr::ScalarFunctionArgs{args: $ARGS, args_data_types: type_array, number_rows: cardinality, return_type: &return_type});
                ...
                Err(expected_error) => {
                    if return_type.is_err() {
                        ...
                        match func.invoke_with_args(datafusion_expr::ScalarFunctionArgs{args: $ARGS, args_data_types: type_array, number_rows: cardinality, return_type: &return_type.unwrap()}) {
                            Ok(_) => assert!(false, "expected error"),
                            Err(error) => {

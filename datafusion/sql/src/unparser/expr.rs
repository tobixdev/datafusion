            ...
            .map(|chunk| {
                let key = match &chunk[0] {
                    Expr::Literal(scalar) => match scalar.value() {
                        ScalarValue::Utf8(Some(s)) => self.new_ident_quoted_if_needs(s.to_string()),
                        _ => return internal_err!("named_struct expects even arguments to be strings, argument had a different type: {:?}", &chunk[0])
                    },
                    _ => return internal_err!("named_struct expects even arguments to be string literals, but received: {:?}", &chunk[0])
                };
